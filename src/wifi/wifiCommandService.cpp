...
WiFiCommandService::WiFiCommandService() {
    addCommand(
        Command("/addSSID", "Add WiFi SSID", WiFiMessageType::addSSID, 1,
                [this](String args) { return WiFiServerService::getInstance().addSSID(args); }));

    addCommand(Command(
        "/addPassword", "Add WiFi Password", WiFiMessageType::addPSD, 1,
    ...
    addCommand(
        Command("/resetWiFiData", "Reset WiFi Data", WiFiMessageType::resetConfig, 1,
