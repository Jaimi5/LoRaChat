...
void WiFiServerService::initWiFi() {
    ...
    if (WiFi.status() == WL_CONNECTED) {
        String status = "WiFi connected: " + WiFi.localIP().toString();
        Log.verboseln("hello");
        initWiFiServer();
    }
...
String WiFiServerService::connectWiFi() {
    ...
    return WiFi.status() == WL_CONNECTED ? "WiFi connected" : "WiFi connection failed";
}
...
String WiFiServerService::startServer() {
    if (serverAvailable)
        return "Server already available";
...
String WiFiServerService::stopServer() {
    if (!serverAvailable)
        return "Server already unavailable";
                    ...
                    if (client.available()) {             // if there's bytes to read from the client,
                        ...
                        if (c == '\n') {                    // if the byte is a newline character
                            ...
                            if (currentLine.length() == 0) {
                                ...
                                client.println("<p>Blink Led </p>");
                                // If the output26State is off, it displays the ON button       
                                // if (output26State == "off") {
                                client.println("<p><a href=\"/blinkLed\"><button class=\"button\">Blink Led</button></a></p>");
                                // }
                                // else {
